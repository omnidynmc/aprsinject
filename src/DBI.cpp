  ...
  void DBI::prepare_queries() {
    add_query("i_last_position",
      "INSERT INTO last_position (packet_id,    callsign_id,    name_id,    icon_id,     maidenhead, latitude, longitude, create_ts) VALUES \
                                 (UNHEX('%0:packet_id'), %1:callsign_id, %2:name_id, %3q:icon_id, %4q:maidenhead, %5:latitude, %6:longitude, %7:create_ts)     \
...
//      mysqlpp::Null<std::string> dir = mysqlpp::quote << NULL_OPTIONPP(aprs, "aprs.packet.dirspd.direction");
      ...
      STRINGTOOL_DEBUG_STRINGS(aprs, stringList, "root");
      while(!stringList.empty()) {
        TLOG(LogInto, << "*** MySQL++ Error{Inject::position}: "
                       << stringList.front() << std::endl);
        stringList.pop_front();
      } // while
  ...
  bool DBI::message(aprs::APRS *aprs) {
    ...
    try {
      ...
      STRINGTOOL_DEBUG_STRINGS(aprs, stringList, "root");
      while(!stringList.empty()) {
        TLOG(LogInto, << "*** MySQL++ Error{Inject::message}: "
                       << stringList.front() << std::endl);
        stringList.pop_front();
      } // while
  ...
  bool DBI::raw(aprs::APRS *aprs) {
    ...
    try {
      ...
      STRINGTOOL_DEBUG_STRINGS(aprs, stringList, "root");
      while(!stringList.empty()) {
        TLOG(LogInfo, << "*** MySQL++ Error{Inject::raw}: "
                       << stringList.front() << std::endl);
        stringList.pop_front();
      } // while
